...
from .base import BaseHandle, TIMEOUT
from .constants import FW_PATH, McuType
from .dfu import PandaDFU
from .spi import PandaSpiHandle, PandaSpiException, PandaSpiMissingAck, PandaProtocolMismatch
...
class Panda:
  ...
  def isotp_recv(self, *, timeout: int = TIMEOUT):
    ...
    while True:
      try:
        dat = self._handle.bulkRead(4, 16384, timeout=timeout)
        break
      except (usb1.USBErrorIO, usb1.USBErrorOverflow):
        logger.error("ISO-TP: BAD RECV, RETRYING")
        time.sleep(0.1)
      except (usb1.USBErrorTimeout, PandaSpiMissingAck):
        return []
