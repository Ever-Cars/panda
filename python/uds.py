...
def get_rx_addr_for_tx_addr(tx_addr, rx_offset=0x8):
  if tx_addr in (0x7DF, 0x18DB33F1):
    raise ValueError("functional addressing is not supported")
...
def _timeout_to_ms(timeout: float) -> int:
...
class UdsClient:
  def __init__(self, panda, tx_addr: int, rx_addr: int | None = None, bus: int = 0, sub_addr: int | None = None, rx_sub_addr: int | None = None,
    ...
    self._isotp_send_timeout_ms = _timeout_to_ms(tx_timeout)
    self._isotp_message_timeout_ms = self._isotp_send_timeout_ms
    self._isotp_transfer_timeout_ms = max(self._isotp_message_timeout_ms, _timeout_to_ms(max(tx_timeout, timeout, response_pending_timeout)))

  def _configure_isotp(self) -> None:
    self._panda.configure_isotp(
      self.bus,
      self.tx_addr,
      self.rx_addr,
      tx_ext_addr=self.sub_addr,
      rx_ext_addr=self.rx_sub_addr,
      message_timeout_ms=self._isotp_message_timeout_ms,
      transfer_timeout_ms=self._isotp_transfer_timeout_ms,
    )
  ...
  def _uds_request(self, service_type: SERVICE_TYPE, subfunction: int | None = None, data: bytes | None = None) -> bytes:
    ...
    self._configure_isotp()
    carlog.debug(f"ISO-TP: REQUEST - {hex(self.tx_addr)} 0x{bytes.hex(req)}")

    # send request, wait for response
    self._panda.isotp_send(req, timeout=self._isotp_send_timeout_ms)
    ...
    while True:
      ...
      while True:
        ...
        resps = self._panda.isotp_recv(timeout=_timeout_to_ms(remaining))
        if len(resps) != 0:
          break

      for resp in resps:
        carlog.debug(f"ISO-TP: RESPONSE - {hex(self.rx_addr)} 0x{bytes.hex(resp)}")
