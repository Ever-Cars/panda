...
It fits the existing panda architecture. However, the ISO-TP periodic hook cannot run from `tick_handler()` in [`board/main.c`](board/main.c), because that path is only 8 Hz. V1 should add a dedicated `1 kHz` ISO-TP periodic hook and treat that as the finest timer resolution used by the ISO-TP engine.
...
- Call `isotp_rx_hook()` from [`board/drivers/fdcan.h`](board/drivers/fdcan.h) after `safety_rx_hook()` succeeds and before the frame is pushed into `can_rx_q`.
